...
import { IconChevronLeft, IconChevronRight, IconHeartFilled, IconHeartPlus, IconMessageCircle, IconMoodAngry, IconMoodSad, IconMoodSmile, IconMoodSurprised, IconSettings, IconThumbUp } from "@tabler/icons-react";
import { useEffect, useMemo, useState } from "react";
import { useChapterByChapterNumber, useChaptersByStory } from "../../hooks/useChapter";
...
import { ReactType, type ReactComment, type ReactTypeValue } from "../../interfaces/ReactComment";
...
const ChapterPage = () => {
    ...
    const { data: chapter } = useChapterByChapterNumber(String(storySlug), Number(chapterNumber));
    ...
    const currentChapterIndex = listChapters?.findIndex((c) => c.chapterNumber === Number(chapterNumber)) ?? -1;
    const prevChapter = currentChapterIndex > 0 ? listChapters?.[currentChapterIndex - 1] : null;
    const nextChapter = currentChapterIndex < (listChapters?.length ?? 0) - 1 ? listChapters?.[currentChapterIndex + 1] : null;
    ...
    const handleSubmitComment = async () => {
        ...
        if (!isLoggedIn) {
            ...
            const submitResult = await CommentService.submitComment(chapter?.id, comment);
            showSuccess(submitResult?.message || "Gửi bình luận thành công.");
            await queryClient.invalidateQueries({ queryKey: ["comments", chapter?.id, commentPage, LIMIT] });
            setComment("");
        }
    }
    ...
    const handleChangeUserFollowStory = async () => {
        ...
        const newStatus = (isFollowStory?.status === "follow" || isFollowStory?.status === "unsend") ? "unfollow" : "follow";

        const submitResult = await FollowStoryService.changeStatusFollowStory(chapter?.id ?? '', newStatus);
        queryClient.invalidateQueries({ queryKey: ["checkUserFollowStory", chapter?.id] });
        showSuccess(submitResult && newStatus === "follow" ? "Theo dõi truyện thành công" : "Hủy theo dõi truyện thành công");
    }
    ...
    const handleUserReact = async (commentId: string, react: ReactTypeValue) => {
        ...
        const submitResult = await ReactCommentService.userReactComment(commentId, chapter?.id, react);
        await queryClient.invalidateQueries({ queryKey: ["comments", chapter?.id, commentPage, LIMIT] });
        await queryClient.invalidateQueries({ queryKey: ["userReactOfChapter", chapter?.id, commentPage] });
    ...
    useEffect(() => {
        ...
        const timer = setTimeout(() => {
            ChapterPageService.userReadChapter(chapter?.storyId, Number(chapterNumber));
        ...
        <Container size="md" py="xl">
            <Stack gap="xl">
                ...
                <Breadcrumbs>
                    ...
                    <Anchor href={`/story/${storySlug}`} className="max-w-48 truncate inline-block align-bottom">
                        {storySlug}
                    </Anchor>
                ...
                <Modal opened={opened} onClose={handleCloseSettings} title="Cấu hình đọc truyện" centered>
                    <Stack gap="lg">
                        {/* Font */}
                        <Select
                            label="Font chữ"
                            ...
                            onChange={(value) =>
                                setSettings({ ...settings, fontFamily: value || "Times New Roman" })
                            }
                        />
                        ...
                        >
                            Đây là đoạn văn bản xem trước để bạn điều chỉnh cấu hình đọc truyện.
                        </Text>
                ...
                <Group justify="center" gap="sm">
                    <HoverCard>
                        ...
                        <HoverCard.Dropdown>
                            <Text size="sm">
                                {isFollowStory?.status === "follow" || isFollowStory?.status === "unsend" ? "Bỏ Theo Dõi" : "Theo Dõi"}
                            </Text>
                        </HoverCard.Dropdown>
                    </HoverCard>
                    <Button
                        disabled={!prevChapter}
                        onClick={() => navigate(`/truyen/${storySlug}/chuong/${prevChapter?.chapterNumber}`)}
                    >
                    ...
                    <Select
                        ...
                        data={chapters}
                        value={chapterNumber}
                        onChange={(value) => navigate(`/truyen/${storySlug}/chuong/${value}`)}
                        chevronColor="blue"
                        allowDeselect={false}
                        w={280}
                    />
                    <Button
                        ...
                        onClick={() => navigate(`/truyen/${storySlug}/chuong/${nextChapter?.chapterNumber}`)}
                    >
                ...
                <Stack gap="md">
                    ...
                    {!comments?.length && <Text c="dimmed" ta="center">Chưa có bình luận nào. Hãy cùng bắt đầu cuộc trò chuyện.</Text>}
                                        ...
                                        >
                                            <HoverCard.Target>
                                                <Button
                                                    ...
                                                    onClick={() => handleUserReact(c.id, userReactMap?.get(c.id)
                                                        ? ReactType.UNLIKE
                                                        : ReactType.LIKE)}
                                                >
                                                    <Group gap={6} >
                                                        {ReactIcons[userReactMap?.get(c.id) as ReactTypeValue || ReactType.UNLIKE]}
                                                        <Text size="sm" fw={500}>
                                            ...
                                            >
                                                <Group gap={6}>
                                                    ...
                                                    <Button
                                                        ...
                                                        onClick={() => handleUserReact(c.id, ReactType.LIKE)}
                                                    >
                                                    ...
                                                    <Button
                                                        ...
                                                        size="xs"
                                                        className="transition-all duration-200 hover:-translate-y-1 hover:scale-125"
                                                        onClick={() => handleUserReact(c.id, ReactType.LOVE)}
                                                    >
                                                        <IconHeartFilled size={18} className="text-pink-500" />
                                                    </Button>
                                                    ...
                                                    <Button
                                                        ...
                                                        size="xs"
                                                        className="transition-all duration-200 hover:-translate-y-1 hover:scale-125"
                                                        onClick={() => handleUserReact(c.id, ReactType.HAHA)}
                                                    >
                                                        <IconMoodSmile size={18} className="text-yellow-500" />
                                                    </Button>
                                                    ...
                                                    >
                                                        <IconMoodSurprised size={18} className="text-orange-500" />
                                                    </Button>
                                                    ...
                                                    <Button
                                                        ...
                                                        size="xs"
                                                        className="transition-all duration-200 hover:-translate-y-1 hover:scale-125"
                                                        onClick={() => handleUserReact(c.id, ReactType.ANGRY)}
                                                    >
