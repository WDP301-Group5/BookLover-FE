...
const StoryCard = ({ story, type = "home" }: StoryCardProps) => {
	...
	if (type === "home") {
		return (
			...
			>
				<Paper shadow="md" radius="sm" className="w-full h-full">
					...
					<div className="px-1 min-h-32 flex flex-col justify-evenly">
						...
						<Text lineClamp={2} size="xs" className="flex gap-1">
							<span>Thể loại: {story.topics?.join(", ")}</span>
						</Text>
					</div>
				</Paper>
			</div>
		...
		return (
			<div
				...
				onClick={() => navigate(storyUrl)}
			>
				<Paper shadow="md" radius="sm" className="w-full h-full p-2">
					<div className="flex gap-2 h-full">
						...
						<div className="px-1 mt-2 w-full flex flex-col justify-between">
							...
							<Text lineClamp={2} size="sm" className="min-h-10">
								{story.description}
							</Text>
						</div>
					</div>
