...
export function DataTablePagination<TData>() {
  ...
  const safePageCount = pageCount === Infinity ? 1 : pageCount;
  const safePageIndex = safePageCount === 0 ? 0 : pageIndex;
      ...
      <Group justify="space-between" align="center" gap="lg" wrap="wrap">
        ...
        <Group gap={"xs"} wrap="nowrap" flex={1}>
          <ActionIcon
            ...
            disabled={!canPreviousPage || safePageCount === 0}
          ...
          <Group gap={4} wrap="nowrap">
            <Select
              value={(safePageIndex + 1).toString()}
              onChange={(value) => table.setPageIndex(Number(value) - 1)}
              data={Array.from({ length: safePageCount }, (_, i) => ({
                value: (i + 1).toString(),
                label: `Trang ${i + 1}`,
              }))}
              size="sm"
          ...
          <ActionIcon
            ...
            disabled={!canNextPage || safePageCount === 0}
